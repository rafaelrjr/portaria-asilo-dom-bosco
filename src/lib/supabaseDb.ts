...
export async function savePerson(person: Person): Promise<void> {
  ...
  const { error } = await supabase.from('persons').upsert({
    ...
    tipo: person.tipo || null,
    parentesco: person.parentesco || null,
    idoso_vinculado: person.idosoVinculado || null,
    observacoes: person.observacoes || null,
    foto: person.foto || null,
    horario_especial: person.horarioEspecial ?? false,
    horario_especial_inicio: person.horarioEspecialInicio || null,
    horario_especial_fim: person.horarioEspecialFim || null,
    dias_permitidos: person.diasPermitidos || null,
  });
  if (error) throw error;
