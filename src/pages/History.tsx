...
export default function History() {
  ...
  function loadData() {
    setVisits(getVisits().sort((a, b) => 
      new Date(b.dataEntrada + 'T' + b.horaEntrada).getTime() - 
      new Date(a.dataEntrada + 'T' + a.horaEntrada).getTime()
    ));
  ...
  const filteredVisits = visits.filter((visit) => {
    ...
    if (searchQuery) {
      ...
      const matchesSearch = 
        visit.pessoa?.nome.toLowerCase().includes(q) ||
        visit.pessoa?.cpf.includes(q);
    ...
    // Resident filter
    if (selectedResident && selectedResident !== 'all' && visit.idosoId !== selectedResident) {
      return false;
    }
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Consulte o histórico completo de visitas
          </p>
        </div>
            ...
            <div className="grid gap-4 md:grid-cols-5">
              ...
              <div className="space-y-2">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    <SelectItem value="all">Todos</SelectItem>
                    <SelectItem value="active">No local</SelectItem>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Visitas
              <Badge variant="secondary">{filteredVisits.length}</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent>
                      ...
                      <TableRow key={visit.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">
                            {getVisitPurposeLabel(visit.proposito)}
                          </Badge>
                        </TableCell>
                        <TableCell>
                          {visit.idoso ? (
                            <span>
                              {visit.idoso.nome}
                              <span className="text-muted-foreground"> (Q.{visit.idoso.quarto})</span>
                            </span>
                          ) : (
                            '-'
                          )}
